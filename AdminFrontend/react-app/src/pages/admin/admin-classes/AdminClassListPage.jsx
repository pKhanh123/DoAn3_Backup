...
export default function AdminClassListPage() {
  ...
  const { data: rawData = {}, isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return adminClassApi.getAll(params).then(r => {
        const d = r.data
        if (d?.success === false || d?.success === true) {
          return { data: d?.data || [], totalCount: d?.totalCount || 0 }
        }
        return { data: Array.isArray(d) ? d : (d?.data || []), totalCount: d?.totalCount || 0 }
      })
    },
    staleTime: 30 * 1000,
  })
  ...
  const { data: majors = [] } = useQuery({
    queryKey: ['majors'],
    queryFn: () => adminClassApi.getMajors().then(r => r.data?.data || r.data || []),
    staleTime: 5 * 60 * 1000,
  })
  const { data: advisors = [] } = useQuery({
    queryKey: ['lecturers-advisor'],
    queryFn: () => adminClassApi.getLecturers().then(r => r.data?.data || r.data || []),
    staleTime: 5 * 60 * 1000,
  })
  const { data: academicYears = [] } = useQuery({
    queryKey: ['academic-years'],
    queryFn: () => adminClassApi.getAcademicYears().then(r => r.data?.data || r.data || []),
    staleTime: 5 * 60 * 1000,
  })

  // Fetch class students
  const fetchClassStudents = (classId) => {
    adminClassApi.getStudents(classId).then(r => {
      const d = r.data
      setClassStudents(d?.success !== false ? (d?.data || d || []) : [])
  ...
  const viewDetail = (c) => {
    setCurrentClass(c)
    fetchClassStudents(c.adminClassId || c.classId)
  ...
  const removeStudentMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (currentClass) fetchClassStudents(currentClass.adminClassId || currentClass.classId)
      queryClient.invalidateQueries({ queryKey: ['admin-classes'] })
    },
                    ...
                    <tr key={c.adminClassId || c.classId}>
                      ...
                      <td>
                        <div className="table-actions">
                          ...
                          <button className="btn btn-sm btn-danger" onClick={() => handleDelete(c.adminClassId || c.classId)}>
                        ...
                        <td>
                          <button className="btn btn-sm btn-danger" onClick={() => {
                            if (window.confirm('Xóa sinh viên khỏi lớp?')) {
                              removeStudentMutation.mutate({ classId: currentClass.adminClassId || currentClass.classId, studentId: s.studentId })
                            }
                          }}>
